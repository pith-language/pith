...
the exact constraint model, persistent pure-computation identity, and incremental engine are still open design work. the rust prototype now tests exact-interface selection, the five effect-category types, arena-owned graph identity, exact in-memory reuse of completed pure applications, validated and stably digested action contracts, the synchronous pure step machine, and an inspectable action-rule/executor boundary. action results deliberately remain non-reusable until cache identity includes rule semantics, the resolved platform, and the relevant policy. persistent caching, invalidation, capability propagation, sandbox enforcement, and concurrent scheduling remain unproved, and proposed decisions stay open until their complete claims have been tested.
