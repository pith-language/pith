...
current evidence: one toolchain runs. `crates/pith-executor-local/tests/real_toolchain.rs` compiles a C source file to an object file through `Engine::run` and the local executor — the engine plans the contract, materializes the compiler and the source, stages them, runs the driver, captures the object, and content-addresses it, with no change to the engine for a tool it did not author. two runs of the same compile in different scratch roots produce byte-identical objects, so this compile is deterministic in the sense 0014 keeps separate from confinement.

three things that slice found, each of which changes what the build library has to be. **a toolchain is not an executable.** `ActionSpec::executable` names one blob and `cc` is a driver that execs `cc1` and `as`; the compile opened 103 distinct paths on one host and 18 on another, none of them declared, and it succeeds only because no landlock ruleset is installed yet. staging a toolchain means staging its closure as declared inputs, which makes it a tree the action depends on. **the seccomp allowlist in 0028 is a sixth of what a compiler needs**, and the missing part is structural rather than incidental — process creation, because the driver is a supervisor. both are recorded in 0028's unresolved section with the measurements. **fine-grained rebuild cannot be shown yet**: the second run of an unchanged compile runs the compiler again, because `finish_action` records every action `NotReusable(ActionCachingDisabled)`. the test asserts that re-execution, so it fails when action caching lands, which is the next thing this milestone needs.
