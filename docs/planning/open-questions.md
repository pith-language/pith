...
- should the synchronous pure step machine be unable to even name effectful steps at the type level? today `PureStep` carries `NeedBlob`/`NeedAction`/`NeedObservation` variants that `evaluate_pure` rejects at runtime (`E-1206`); a separate pure-only step type would make that a compile property, which is what decision 0022's "structurally pure core" already claims
...
- who may write a content store, and how many stores a machine holds once M-5b activates a composed tree onto it. the settled half is that activation makes a store's contents the running system, so an account able to write the store can modify a binary root will later execute, which is why `/nix/store` is root-owned on single-user machines as well as shared ones. two findings sit under the open half. the first is that a daemon has two jobs and nix gives them to one process: `nix-daemon` brokers privilege because unprivileged users build *into* the shared root-owned store, while a long-running language server or watch mode is a latency device needing no authority its caller lacks, and the two roles should not be merged by accident. the second is that verification on read does not survive to exec, since a blob can be verified and then rewritten before the kernel maps it. content addressing therefore removes the need for a broker and leaves the need for ownership at the activation boundary standing; `get_blob` and `get_tree` already re-derive an identity and refuse a mismatch, and no digest checks whether the right identity was named, which is the claim boundary 0031's environment test and `PlatformRequirement::Exact` guard from the other side. what stays open is the topology. a user-owned store per evaluating account beside a root-owned system store holds the activated closure twice on a single-user machine, and a hardlink cannot recover the duplicate without letting the owning user rewrite what root executes, so the cost is structural and is a large part of what nix's shared store and its broker buy. M-5b decides, against nixos's multi-user installation and bazel's disk cache, and [the cli surface](cli-surface.md) leaves both roots as parameters so nothing is foreclosed
...
- can observations participate in incremental computation without making pure results time-dependent? ([0060](../decisions/0060-observation-identity-and-freshness.md) answers the cache case: a pure consumer records the observation attempt and is admitted only after the same observer attests the same revision; the observation itself never enters the durable reusable-result index. 0012's projection of those recorded revisions into an executable plan remains open for M-5b and M-6)
