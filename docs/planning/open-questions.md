...
- when does a dependency edge enter the package line? ([0045](../decisions/0045-a-locked-source-becomes-a-built-artifact.md) defers the edge deliberately: the local index carries no requirements — one line per version — so an edge would be fabricated by a fixture rather than read from a source. the first index format that carries requirements is what measures whether the candidate record the universe spells is the one a real registry answers with, and the edge is that round's first item)
