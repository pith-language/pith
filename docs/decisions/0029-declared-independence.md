...
the executor now sees concurrent calls. `Executor::execute` takes `&self` and was already required to be `Sync`, so this imposes no new bound, but an executor with shared mutable state now has to say what it does about it. the first-party local executor (0028) has none in its own types: each action gets its own scratch root. it shares a process, though, and that turned out to be enough to couple two actions that share nothing else — a forked child holds a copy of every descriptor its parent had open, including the write handle a sibling action is using to stage *its* executable, so `execve` fails with `ETXTBSY` on an executable no one else has any interest in. under load it took out one action in a run of twenty-four about fifteen percent of the time. the executor now excludes the two from each other with a process-wide gate: writing a staged executable takes the shared side, forking takes the exclusive one, so a child cannot be forked while an executable is being written and there is nothing for it to inherit. the general point is that "no shared state" is a claim about types, and the file-descriptor table is shared regardless — which is also why the gate is process-wide rather than owned by an executor.
