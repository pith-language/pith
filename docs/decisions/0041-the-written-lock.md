...
publication follows 0024's content discipline as the store implements it. a temporary file named for this writer is created exclusively, so two writers cannot share one. it is flushed and renamed into place, the destination directory is flushed after the rename so the rename itself survives a crash, and the temporary file is removed on every failure path. only a solved answer yields a written lock. an unsatisfiable, underdetermined, or exhausted resolution writes nothing and the existing file stands, on the same ground 0024 uses when a failed reevaluation publishes no partial dependency set: the file records selections, and none of the three is one.
