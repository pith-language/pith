...
`NeedAll`'s independence declaration and `NeedBlob`/`NeedAction`/`NeedObservation` are ir constructs at the same level. the step protocol is the ir's effect vocabulary at the kernel boundary; 0019's categories arrive in the ir as the types of what the steps request, which they already are in the protocol (`Request<Pure>`, `Request<Action>`, `Request<Observation>`). [0060](0060-observation-identity-and-freshness.md) adds the observation constructor before M-11 fixes this record's encoding; mutation and opaque still have no step protocol.
