...
rejected because none of 0013's defining pressure is present, with one honesty qualification: a package version's immutability is domain policy, not a property of the identifier. the context above already says so about npm, and a domain that republishes different bytes under one name is exactly the failure the lock binding exists to catch. what pith does when a domain does not honor immutability is drift detection: the binding is intrinsic content identity checked against bytes, so content that no longer matches it fails verification and surfaces as a policy failure, never as an identity transition. 0013's question is the other one, what happens to an object that legitimately mutates while remaining the same owned thing, and a package version is never that object, because pith claims no continuity across a content change — different content under the same coordinates is either a new resolution to record in the lock or drift to report. and the registry entry itself is 0005's external identity, a name an external system assigns, recorded in provenance rather than promoted to the package's own identity. stretching the managed-object type here would erode the mutable-immutable distinction it exists to carry, the same collapse 0013 rejected when it declined to fold managed objects into external identity.
