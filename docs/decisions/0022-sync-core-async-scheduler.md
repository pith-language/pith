...
the driver now schedules independent work concurrently. it holds a set of evaluation chains, each a synchronous stack on the same step machine, and overlaps them at the points where they park: on a fan-out group, or on an action whose executor has not returned. the synchronous core is unchanged by this, which is the claim this decision makes — a chain never awaits, and only a parked chain yields, so a running chain's stack cannot move under it. the width the scheduler exploits is declared rather than inferred; [0029](0029-declared-independence.md) records why and what it costs. a test holds three actions at a barrier that releases only when all three have arrived, so an engine that ran them one at a time could not complete it.

cancellation is prototyped for the case the scheduler owns: a caller stopping a run. `Engine::run_cancellable` takes a signal the engine polls at scheduling boundaries — before stepping a chain, and after an action returns — and a host backs it with whatever it already has, so the runtime stays out of that signature the same way it stays out of evaluation. propagation needs no traversal: the scheduler already holds every live chain and every in-flight action, so stopping a run is a walk over what it is holding rather than a search through the graph. this is why the parent-pointer index an earlier sketch called for is absent — the scheduler's own bookkeeping is the descendant set.
