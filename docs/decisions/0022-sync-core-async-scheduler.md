...
`Pure` rule bodies run as synchronous frames and yield typed requests. an `Action` rule plans an inert contract containing its executable content identity, arguments, inputs, outputs, environment, platform requirement, capabilities, and network policy. callers can query that plan and its stable contract digest without executing it. an async executor adapter is the only surface that performs the action. it returns captured outputs and a report containing the selected platform, access-verification mechanism, and capability use; the action rule derives the typed semantic result from those outputs. the engine validates the report against the contract and retains it as provenance, but does not treat an executor's claim as sufficient proof for cache reuse.
