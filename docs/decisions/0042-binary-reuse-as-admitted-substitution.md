...
the environment leg: the realization coordinates the offer claims — platform and toolchain, the request inputs a realization's identity already covers (0039) — equal this run's. the toolchain side is the value the run's build requests carry, xylem's toolchain value, rather than a label the caller spells beside it: a comparison against a caller-supplied string is a leg nothing ties to the build it guards, and the fixture that motivated this amendment admitted under `gcc-13` while building under `/nix/store/cc` with nothing to notice. comparing the values ties the leg to the same input the requests name. a binary for another platform or another toolchain is a different realization and fails before authorization is even asked. this leg is what makes the substitution a realization-level fact rather than a package-level one, and it is where a cross-compiling environment says which binaries are its own.
