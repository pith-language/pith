...
that build library is `crates/xylem`. `Toolchain::discover` is the public API: it resolves the driver on `PATH`, reduces it to its store root, walks the closure with `nix path-info --recursive`, asks the driver where `cc1` lives, and returns a `Toolchain` carrying the driver path, the closure, and the two search paths. it declines outside `/nix/store` (returning `DiscoveryError::Incomplete`) rather than falling back to the `LD_TRACE_LOADED_OBJECTS` trace, which covers what the loader opens and nothing a program opens later; a partial answer would fail on an undeclared read and say nothing about the engine. the test support in `crates/pith-executor-local/tests/support` that stood in for it is unchanged — xylem is not a dependency of the executor tests, and rewiring it is follow-up that adds no evidence.
...
this section originally sequenced seccomp first and action caching second. that ordering did not survive contact: caching landed first, as [0031](0031-action-cache-identity.md), and 0031 then removed the dependency between them by making `AccessVerification` an admission floor rather than key material, so an entry recorded under `Observed` stays findable and becomes unacceptable to a caller who later demands `Prevented`. the two increments are independent and the caching one is done. `the_same_compile_is_served_from_the_reusable_index` is the test that was predicted to flip, and it has.

the seccomp allowlist therefore blocks nothing and remains open on its own terms. 0028's measurement of the 45-syscall list a real compile issues is drawn from the same trace that produced the 106-path measurement here, and 0028 also records that seccomp is the wrong instrument for the network claim, because glibc's name-service switch opens an `AF_UNIX` socket during an ordinary compile. when a filter lands, `AccessVerification` moves from `Observed` to `Prevented` for the local executor; whether that filter should be a full allowlist or only an address-family restriction belongs with the network-namespace design both records defer.
