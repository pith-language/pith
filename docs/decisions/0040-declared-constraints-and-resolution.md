...
what is shared, and deliberately small, is the protocol every domain's model satisfies. a solver request names four values: the constraint set, the candidate universe with the provenance of every candidate, the preference order, and the search budget in deterministic units. a solver answer names two: the choice, as one candidate per constrained subject, and the explanation, as a value. the candidates are 0025-queryable evidence rather than ambient repository state, which is what makes an answer reproducible under the same universe — the property the research record already requires of locks.
...
the determinism contract is the load-bearing clause. a solver body declares that its answer is a pure function of its inputs, which makes the result cacheable under its key, reproducible across processes on 0024's terms, and explainable by input diff alone. within that contract the search may backtrack, learn, and order itself however it likes — the engine never sees the interior, only the request and the answer, which is the same epistemic position it holds toward every host rule.

the search budget is a fourth declared input, denominated in deterministic units — decisions taken, conflicts learnt — never wall-clock. a budget-exhausted answer is then a function of the inputs like any other and may be cached, which a time-denominated budget would break twice over: the answer would vary with the machine, and a run served a recorded exhaustion from a slower machine would skip a search that would have found a solution. exhaustion stays excluded from what an answer may be read as — it says the budget ran out, never that no solution exists — on the same ground 0022 drew between a failure and an interruption: one is a fact about the problem, the other a fact about the run.
