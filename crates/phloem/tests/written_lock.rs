...
use pith_core::{ActionComputationKey, Pure, PureComputationKey, Request, Value};
...
/// describes; two resolutions over two such substrates is what the
/// byte-identity claim needs, so no engine or state is shared there.
